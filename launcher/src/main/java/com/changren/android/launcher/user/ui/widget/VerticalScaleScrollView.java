...
public class VerticalScaleScrollView extends BaseScaleView {
    ...
    protected void onDrawScale(Canvas canvas, Paint paint) {
        ...
        for (int i = 0, k = mMin; i <= mMax - mMin; i++) {
            if (i % 5 == 0) { //整值
                canvas.drawLine(mRectWidth, i * mScaleMargin, mRectWidth - mScaleMaxHeight, i * mScaleMargin, paint);
                if (i % 10 == 0) {
                    //整值文字
                    canvas.drawText(String.valueOf(k), mRectWidth - mScaleMaxHeight - mScaleHeight * 2, i * mScaleMargin + paint.getTextSize() / 3, paint);
                    k += 10;
                }
            } else {
                canvas.drawLine(mRectWidth, i * mScaleMargin, mRectWidth - mScaleHeight, i * mScaleMargin, paint);
            }
        }
    }
    ...
    protected void onDrawPointer(Canvas canvas, Paint paint) {
        ...
        canvas.drawLine(mRectWidth, countScale * mScaleMargin + finalY,
                mRectWidth - mScaleMaxHeight - mScaleHeight, countScale * mScaleMargin + finalY, paint);
