...
//画分割线
            ...
            if (button.isFocus()) {
                ...
                paint.setAlpha(builder.sheetPressAlpha);
            ...
            if (button.isFocus()) {
                ...
                mDrawables.getPaint().setAlpha(builder.sheetPressAlpha);
...
//        paint.getTextBounds(text, 0, text.length(), textRect);
        ...
        switch (currentType) {
            case PROMPT_ALERT_WARN:
                ...
                break;
            default:
                popWidth = Math.max(300 * density, textRect.width() + pad * 2);
...
//        promptDialog = null;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (currentType == PROMPT_ALERT_WARN) {
            ...
            if (builder.cancelAble && event.getAction() == MotionEvent.ACTION_UP && !roundTouchRect.contains(x, y)) {
            ...
            if (event.getAction() == MotionEvent.ACTION_UP) {
                if ((drawableClose != null && drawableClose.getBounds()
                        .contains((int) event.getX() - transX, (int) event.getY() - transY)) || builder.cancelAble) {
                    promptDialog.dismiss();
                } else if (getDrawable() !=
